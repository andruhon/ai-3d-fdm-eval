 ...
 * 4. Model is positioned upsides-down
 */
export const trapezoidalRivetTask: Task = {
  name: "trapezoidal-rivet",
  description:
    "Generate a trapezoidal head rivet (pan head) plug designed for 3D printing on its head",
  type: "zero-shot",
...
2. The part will be printed standing on its head (inverted), so the head should be at the bottom in the model
3. Model the rivet with the head at Z=0 (bottom) and shaft pointing upward
4. The shaft and head should be a single unified object

Create a file called "trapezoidal-rivet-ud.scad" with the complete OpenSCAD code.
  ...
  async validate(outputDir: string): Promise<TaskResult> {
    ...
    if (!existsSync(pngFile)) {
      return {
        ...
        success: false,
        error:
          "PNG file was not generated despite successful render (default view)",
        outputPath: outputDir,
        metadata: {
          stdout: renderResult.stdout,
          stderr: renderResult.stderr,
    ...
    console.log(
      `Rendering bottom isometric view: ${scadFile} -> ${pngBottomFile}`,
    );
    const renderBottomResult = await renderOpenSCADBottomIsometric(
      scadFile,
      pngBottomFile,
    );
    ...
    if (!existsSync(pngBottomFile)) {
      return {
        ...
        success: false,
        error:
          "PNG file was not generated despite successful render (bottom isometric view)",
        outputPath: outputDir,
        metadata: {
          defaultView: {
            success: true,
